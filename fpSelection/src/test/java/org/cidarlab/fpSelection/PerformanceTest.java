...
public class PerformanceTest {
    ...
    private static String fp8spectrafp = basefp + "subset" + Utilities.getSeparater() + "spectra.csv";
    private static String fp8brightnessfp = basefp + "subset" + Utilities.getSeparater() + "brightness.csv";
    
    private static String fp12spectrafp = basefp + "main" + Utilities.getSeparater() + "spectra.csv";
    private static String fp12brightnessfp = basefp + "main" + Utilities.getSeparater() + "brightness.csv";
    
    private static ThreadLocal<Random> threadrandom = Utilities.threadRandom(0);
    
    private static int exhaustiveLim = 100;
    ...
    private static void getSARank(int n, Map<String,Fluorophore> fpmap, Cytometer cyto, String cytoname, List<Map.Entry<List<SelectionInfo>, SNR>> exresults) throws IOException, InterruptedException {

        System.out.println("Starting Simulated Annealing Run");
        String rootfp = basefp + "performance" + Utilities.getSeparater();

        //List<String> ranks = new ArrayList<>();

        Map<String,Integer> ranks = new HashMap<>();
        for(int i=0;i<exhaustiveLim;i++){
            ranks.put(selectionInfoToString(exresults.get(i).getKey()),i);
        }


        //for(Map.Entry<List<SelectionInfo>, SNR> entry: exresults){            
        //    ranks.add(selectionInfoToString(entry.getKey()));
        //}

        List<String> lines = new ArrayList<>();
        String header = "Run,Rank";
        for(int i=0;i<n;i++){
            header += ",Fluorophore,Laser,Detector";
        }
        lines.add(header);
        ...
        for(int i=0;i<200;i++){
            ...
            int seedstart = (i * 50);
            ThreadLocal<Random> random = Utilities.threadRandom(seedstart);
            List<SelectionInfo> best = TestUtilities.runSeededSA(fpmap, cyto, n, 10000, 0.001, random);
            SNR bestSNR = new SNR(best);


            
            for(int j = seedstart+1; j < (seedstart+50);j++){
                random = Utilities.threadRandom(j);
                List<SelectionInfo> next = TestUtilities.runSeededSA(fpmap, cyto, n, 10000, 0.001, random);
            ...
            String str = selectionInfoToString(best);
            
            int rank = -1;
            if(ranks.containsKey(str)){
                rank = ranks.get(str);
            }
            lines.add(i + "," + rank + "," +  TestUtilities.toString(best));
        }

        Utilities.writeToFile(rootfp + cytoname + "_sa" + n + "fp.csv",lines);
    }

    private static void getSARank(int n, Map<String,Fluorophore> fpmap, Cytometer cyto, String cytoname) throws IOException, InterruptedException {
        List<Map.Entry<List<SelectionInfo>, SNR>> exresults = exhaustiveRunList(n,fpmap,cyto);
        getSARank(n,fpmap,cyto,cytoname,exresults);
    }

    private static void getHCRank(int n, Map<String,Fluorophore> fpmap, Cytometer cyto, String cytoname, List<Map.Entry<List<SelectionInfo>, SNR>> exresults) throws IOException, InterruptedException {

        System.out.println("Starting Hill Climbing Run");
        String rootfp = basefp + "performance" + Utilities.getSeparater();
        ...
        for(int i=0;i<200;i++){
            ...
            int seedstart = (i * 50);
            ThreadLocal<Random> random = Utilities.threadRandom(seedstart);
            List<SelectionInfo> best = TestUtilities.runSeededHC(fpmap, cyto, n, 10000, random);
            SNR bestSNR = new SNR(best);


            for(int j = seedstart+1; j < (seedstart+50);j++){
                random = Utilities.threadRandom(j);
                List<SelectionInfo> next = TestUtilities.runSeededHC(fpmap, cyto, n, 10000, random);
                SNR nextSNR = new SNR(next);
                if (nextSNR.greaterThan(bestSNR)) {
                    bestSNR = nextSNR;
                    best = new ArrayList<>(next);
        ...
        Utilities.writeToFile(rootfp + cytoname + "_hc" + n + "fp.csv",lines);
    }

    private static void getHCRank(int n, Map<String,Fluorophore> fpmap, Cytometer cyto, String cytoname) throws IOException, InterruptedException {
        List<Map.Entry<List<SelectionInfo>, SNR>> exresults = exhaustiveRunList(n,fpmap,cyto);
        getHCRank(n,fpmap,cyto,cytoname,exresults);
    }

    
    private static void getStochasticRank(int n, Map<String,Fluorophore> fpmap, Cytometer cyto, String cytoname) throws IOException, InterruptedException {
        List<Map.Entry<List<SelectionInfo>, SNR>> exresults = exhaustiveRunList(n,fpmap,cyto);
        getSARank(n,fpmap,cyto,cytoname,exresults);
        getHCRank(n,fpmap,cyto,cytoname,exresults);
    }
    ...
    @Test
    public void testCytoFlexRanks() throws IOException, InterruptedException {
        System.out.println("Cytometer: CytoFlex");
        Cytometer harvardCytoflex = fpFortessaParse.parse(harvardCytoFlexfp, false);
        Map<String, Fluorophore> fp8map = fpSpectraParse.parse(fp8spectrafp);
        Map<String, Fluorophore> fp12map = fpSpectraParse.parse(fp12spectrafp);
        for(int i=2;i<=5;i++){
            getStochasticRank(i,fp8map,harvardCytoflex,"CytoFlex8fp");
            getStochasticRank(i,fp12map,harvardCytoflex,"CytoFlex12fp");
        }
    ...
    @Test
    public void testMacsquantRanks() throws IOException, InterruptedException {
        Cytometer harvardMacsquant = fpFortessaParse.parse(harvardMacsquantfp, false);
        Map<String, Fluorophore> fp8map = fpSpectraParse.parse(fp8spectrafp);
        Map<String, Fluorophore> fp12map = fpSpectraParse.parse(fp12spectrafp);
        for(int i=2;i<=5;i++){
            getStochasticRank(i,fp8map,harvardMacsquant,"Macsquant8fp");
            getStochasticRank(i,fp12map,harvardMacsquant,"Macsquant12fp");
        }

    }
    
    @Test
    public void testFortessaRanks() throws IOException, InterruptedException {
        Cytometer harvardFortessa = fpFortessaParse.parse(harvardFortessafp, false);
        Map<String, Fluorophore> fp8map = fpSpectraParse.parse(fp8spectrafp);
        Map<String, Fluorophore> fp12map = fpSpectraParse.parse(fp12spectrafp);
        for(int i=2;i<=5;i++){
            getStochasticRank(i,fp8map,harvardFortessa,"Fortessa8fp");
            getStochasticRank(i,fp12map,harvardFortessa,"Fortessa12fp");
        }
    ...
    public void getRuntimes() throws IOException, InterruptedException{
        ...
        Map<String, Fluorophore> caseStudySpectralMap = fpSpectraParse.parse(fp8spectrafp);
        fpSpectraParse.addBrightness(new File(fp8brightnessfp), caseStudySpectralMap);
